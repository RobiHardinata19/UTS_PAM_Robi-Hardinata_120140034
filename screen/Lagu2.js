...
const Lagu2 = ({ navigation }) => {

  const [sound, setSound] = React.useState();

  async function playSound() {
    console.log('Loading Sound');
    const { sound } = await Audio.Sound.createAsync( require('../assets/GarudaPancasila.mp3')
    );
    setSound(sound);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
          ...
          <Text style={styles.heading2}>Robi Hardinata</Text>
      </View>
      <View style={styles.detail}>
         <Text style={styles.memutar}>Sedang Memutar</Text>
            <Image
                style={{width: 300, height:300, borderRadius:10, marginTop:25}}
                source={require('../assets/detailLagu2.png')}
            />
        <Text style={styles.judulLagu}>Garuda Pancasila</Text>
        <Text style={styles.pencipta}>Prohar Sudharnoto</Text>
      </View>
