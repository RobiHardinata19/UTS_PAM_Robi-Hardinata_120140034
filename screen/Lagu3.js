...
const Lagu3 = ({ navigation }) => {

  const [sound, setSound] = React.useState();

  async function playSound() {
    console.log('Loading Sound');
    const { sound } = await Audio.Sound.createAsync( require('../assets/IndonesiaPusaka.mp3')
    );
    setSound(sound);
      ...
      <View style={styles.detail}>
         <Text style={styles.memutar}>Sedang Memutar</Text>
            <Image
                ...
                source={require('../assets/detailLagu3.png')}
            />
        <Text style={styles.judulLagu}>Indonesia Pusaka</Text>
        <Text style={styles.pencipta}>Ismail Marzuki</Text>
      </View>
